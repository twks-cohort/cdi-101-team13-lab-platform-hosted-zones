...
provider "aws" {
  alias  = "subdomain_prod_cdicohorts_thirteen"
  region = "us-east-2"
  assume_role {
...
module "subdomain_prod_cdicohorts_thirteen" {
  source  = "terraform-aws-modules/route53/aws//modules/zones"
  version = "2.0.0"
  create  = true

  providers = {
    aws = aws.subdomain_prod_cdicohorts_thirteen
  }

  zones = {
    "prod.${local.domain_cdicohorts_thirteen}" = {
      tags = {
        cluster = "nonprod"
      }
    }
  ...
  tags = {
    pipeline = "cdi101team130-lab-platform-hosted-zones"
  }
}

# Create a zone delegation in the top level domain for this subdomain
module "subdomain_zone_delegation_prod_cdicohorts_thirteen" {
  ...
  records = [
    {
      ...
      records         = lookup(module.subdomain_prod_cdicohorts_thirteen.route53_zone_name_servers,"prod.${local.domain_cdicohorts_thirteen}")
    }
  ]

  depends_on = [module.subdomain_prod_cdicohorts_thirteen]
}